...
class Trakt():
    ...
    @staticmethod
    def jsonRequest(method, req, args={}, returnStatus=False, anon=False, conn=False, daemon=False, passVersions=False):
    ...
    def accountCreate(email, *args, **argd):
        data = traktJsonRequest('POST', '/account/create/%%API_KEY%%', {'email': email}, *args, **argd)
        if data == None:
    ...
    def accountTest(*args, **argd):
        data = traktJsonRequest('POST', '/account/test/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def calendarPremieres(date=None, days=None, *args, **argd):
        ...
        if date is not None:
            ...
            if days is not None:
                ext += "/"+str(days)
        data = traktJsonRequest('GET', '/calendar/premieres.json/%%API_KEY%%'+ext, *args, **argd)
        if data == None:
    ...
    def calendarShows(date=None, days=None, *args, **argd):
        ...
        if date is not None:
            ...
            if days is not None:
                ext += "/"+str(days)
        data = traktJsonRequest('GET', '/calendar/shows.json/%%API_KEY%%'+ext, *args, **argd)
        if data == None:
    ...
    def friendsAdd(friend, *args, **argd):
        data = traktJsonRequest('POST', '/friends/add/%%API_KEY%%', {'friend': friend}, *args, **argd)
        if data == None:
    ...
    def friendsAll(*args, **argd):
        data = traktJsonRequest('POST', '/friends/all/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def friendsApprove(friend, *args, **argd):
        data = traktJsonRequest('POST', '/friends/approve/%%API_KEY%%', {'friend': friend}, *args, **argd)
        if data == None:
    ...
    def friendsDelete(friend, *args, **argd):
        data = traktJsonRequest('POST', '/friends/delete/%%API_KEY%%', {'friend': friend}, *args, **argd)
        if data == None:
    ...
    def friendsDeny(friend, *args, **argd):
        data = traktJsonRequest('POST', '/friends/deny/%%API_KEY%%', {'friend': friend}, *args, **argd)
        if data == None:
    ...
    def friendsRequests(*args, **argd):
        data = traktJsonRequest('POST', '/friends/requests/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def genresMovies(*args, **argd):
        data = traktJsonRequest('GET', '/genres/movies.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def genresShows(*args, **argd):
        data = traktJsonRequest('GET', '/genres/shows.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def listsAdd(name, description, privacy, *args, **argd):
        data = traktJsonRequest('POST', '/lists/add/%%API_KEY%%', {'name': name, 'description': description, 'privacy': privacy}, *args, **argd)
        if data == None:
    ...
    def listsDelete(slug, *args, **argd):
        data = traktJsonRequest('POST', '/lists/delete/%%API_KEY%%', {'slug': slug}, *args, **argd)
        if data == None:
    ...
    def listsItemsAdd(slug, items, *args, **argd):
        data = traktJsonRequest('POST', '/lists/items/add/%%API_KEY%%', {'slug': slug, 'items': items}, *args, **argd)
        if data == None:
    ...
    def listsItemsDelete(slug, items, *args, **argd):
        data = traktJsonRequest('POST', '/lists/items/delete/%%API_KEY%%', {'slug': slug, 'items': items}, *args, **argd)
        if data == None:
    ...
    def listsUpdate(slug, name=None, description=None, privacy=None, *args, **argd):
        ...
        data = traktJsonRequest('POST', '/lists/update/%%API_KEY%%', data, *args, **argd)
        if data == None:
    ...
    def movieCancelWatching(*args, **argd):
        data = traktJsonRequest('POST', '/movie/cancelwatching/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def movieScrobble(imdbId, title, year, duration, progress, tmdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/movie/scrobble/%%API_KEY%%', {'imdb_id': imdbId, 'title': title, 'year': year, 'duration': duration, 'progress': progress, 'tmdb_id': tmdbId}, *args, **argd)
        if data == None:
    ...
    def movieSeen(movies, *args, **argd):
        data = traktJsonRequest('POST', '/movie/seen/%%API_KEY%%', {'movies': movies}, *args, **argd)
        if data == None:
    ...
    def movieShouts(title, *args, **argd):
        data = traktJsonRequest('GET', '/movie/shouts.json/%%API_KEY%%/'+str(title), *args, **argd)
        if data == None:
    ...
    def movieSummary(title, *args, **argd):
        data = traktJsonRequest('GET', '/movie/summary.json/%%API_KEY%%/'+str(title), *args, **argd)
        if data == None:
    ...
    def movieUnseen(movies, *args, **argd):
        data = traktJsonRequest('POST', '/movie/unseen/%%API_KEY%%', {'movies': movies}, *args, **argd)
        if data == None:
    ...
    def movieUnwatchlist(movies, *args, **argd):
        data = traktJsonRequest('POST', '/movie/unwatchlist/%%API_KEY%%', {'movies': movies}, *args, **argd)
        if data == None:
    ...
    def movieWatching(imdbId, title, year, duration, progress, tmdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/movie/watching/%%API_KEY%%', {'imdb_id': imdbId, 'title': title, 'year': year, 'duration': duration, 'progress': progress, 'tmdb_id': tmdbId}, *args, **argd)
        if data == None:
    ...
    def movieWatchingNow(title, *args, **argd):
        data = traktJsonRequest('GET', '/movie/watchingnow.json/%%API_KEY%%', {'title': title}, *args, **argd)
        if data == None:
    ...
    def movieWatchlist(movies, *args, **argd):
        data = traktJsonRequest('POST', '/movie/watchlist/%%API_KEY%%', {'movies': movies}, *args, **argd)
        if data == None:
    ...
    def moviesTrending(*args, **argd):
        data = traktJsonRequest('GET', '/movie/trending.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def rateEpisode(tvdbId, title, year, season, episode, rating, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/rate/episode/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'season': season, 'episode': episode, 'rating': rating, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def rateMovie(imdbId, title, year, rating, tmdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/rate/movie/%%API_KEY%%', {'imdb_id': imdbId, 'title': title, 'year': year, 'rating': rating, 'tmdb_id': tmdbId}, *args, **argd)
        if data == None:
    ...
    def rateShow(tvdbId, title, year, rating, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/rate/show/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'rating': rating, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def recommendationsMovies(genre=None, startYear=None, endYear=None, *args, **argd):
        ...
        data = traktJsonRequest('POST', '/recommendations/movies/%%API_KEY%%', data, *args, **argd)
        if data == None:
    ...
    def recommendationsMoviesDismiss(imdbId, title, year, tmdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/recommendations/movies/dismiss/%%API_KEY%%', {'imdb_id': imdbId, 'title': title, 'year': year, 'tmdb_id': tmdbId}, *args, **argd)
        if data == None:
    ...
    def recommendationsShows(genre=None, startYear=None, endYear=None, *args, **argd):
        ...
        if startYear is not None: data['startYear'] = startYear
        if endYear is not None: data['endYear'] = endYear
        data = traktJsonRequest('POST', '/recommendations/shows/%%API_KEY%%', data, *args, **argd)
        if data == None:
    ...
    def recommendationsShowsDismiss(tvdbId, title, year, *args, **argd):
        data = traktJsonRequest('POST', '/recommendations/shows/dismiss/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year}, *args, **argd)
        if data == None:
    ...
    def searchEpisodes(query, *args, **argd):
        data = traktJsonRequest('GET', '/search/episodes.json/%%API_KEY%%/'+str(query), *args, **argd)
        if data == None:
    ...
    def searchMovies(query, *args, **argd):
        data = traktJsonRequest('GET', '/search/movies.json/%%API_KEY%%/'+str(query), *args, **argd)
        if data == None:
    ...
    def searchPeople(query, *args, **argd):
        data = traktJsonRequest('GET', '/search/people.json/%%API_KEY%%/'+str(query), *args, **argd)
        if data == None:
    ...
    def searchShows(query, *args, **argd):
        data = traktJsonRequest('GET', '/search/shows.json/%%API_KEY%%/'+str(query), *args, **argd)
        if data == None:
    ...
    def searchUsers(query, *args, **argd):
        data = traktJsonRequest('GET', '/search/users.json/%%API_KEY%%/'+str(query), *args, **argd)
        if data == None:
    ...
    def shoutEpisode(tvdbId, title, year, season, episode, shout, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/shout/episode/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'season': season, 'episode': episode, 'shout': shout, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def shoutMovie(imdbId, title, year, shout, tmdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/shout/movie/%%API_KEY%%', {'imdb_id': imdbId, 'title': title, 'year': year, 'shout': shout, 'tmdb_id': tmdbId}, *args, **argd)
        if data == None:
    ...
    def shoutShow(tvdbId, title, year, shout, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/shout/show/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'shout': shout, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showCancelWatching(*args, **argd):
        data = traktJsonRequest('POST', '/show/cancelwatching/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def showEpisodeLibrary(tvdbId, title, year, episodes, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/library/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'episodes': episodes, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showEpisodeSeen(tvdbId, title, year, episodes, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/seen/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'episodes': episodes, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showEpisodeShouts(title, season, episode, *args, **argd):
        data = traktJsonRequest('GET', '/show/episode/shouts.json/%%API_KEY%%/'+str(title)+"/"+str(season)+"/"+str(episode), *args, **argd)
        if data == None:
    ...
    def showEpisodeSummary(title, season, episode, *args, **argd):
        data = traktJsonRequest('GET', '/show/episode/summary.json/%%API_KEY%%/'+str(title)+"/"+str(season)+"/"+str(episode), *args, **argd)
        if data == None:
    ...
    def showEpisodeUnlibrary(tvdbId, title, year, episodes, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/unlibrary/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'episodes': episodes, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showEpisodeUnseen(tvdbId, title, year, episodes, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/unseen/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'episodes': episodes, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showEpisodeUnwatchlist(tvdbId, title, year, episodes, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/unwatchlist/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'episodes': episodes, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showEpisodeWatchingNow(title, season, episode, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/watchingnow/%%API_KEY%%/'+str(title)+"/"+str(season)+"/"+str(episode), *args, **argd)
        if data == None:
    ...
    def showEpisodeWatchlist(tvdbId, title, year, episodes, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/episode/watchlist/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'episodes': episodes, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showLibrary(tvdbId, title, year, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/library/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showScrobble(tvdbId, title, year, season, episode, duration, progess, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/scrobble/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'season': season, 'episode': episode, 'duration': duration, 'progess': progess, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showSeason(title, season, *args, **argd):
        data = traktJsonRequest('GET', '/show/season.json/%%API_KEY%%/'+str(title)+"/"+str(season), *args, **argd)
        if data == None:
    ...
    def showSeasonLibrary(tvdbId, title, year, season, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/season/library/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'season': season, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showSeasonSeen(tvdbId, title, year, season, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/season/seen/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'season': season, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showSeasons(title, *args, **argd):
        data = traktJsonRequest('GET', '/show/seasons.json/%%API_KEY%%/'+str(title), *args, **argd)
        if data == None:
    ...
    def showSeen(tvdbId, title, year, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/seen/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showShouts(title, *args, **argd):
        data = traktJsonRequest('GET', '/show/shouts.json/%%API_KEY%%/'+str(title), *args, **argd)
        if data == None:
    ...
    def showSummary(title, extended=None, *args, **argd):
        ...
        data = traktJsonRequest('GET', '/show/summary.json/%%API_KEY%%/'+str(title)+ext, *args, **argd)
        if data == None:
    ...
    def showUnlibrary(tvdbId, title, year, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/unlibrary/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showUnwatchlist(shows, *args, **argd):
        data = traktJsonRequest('POST', '/show/unwatchlist/%%API_KEY%%', {'shows': shows}, *args, **argd)
        if data == None:
    ...
    def showWatching(tvdbId, title, year, season, episode, duration, progess, imdbId=None, *args, **argd):
        data = traktJsonRequest('POST', '/show/watching/%%API_KEY%%', {'tvdb_id': tvdbId, 'title': title, 'year': year, 'season': season, 'episode': episode, 'duration': duration, 'progess': progess, 'imdb_id': imdbId}, *args, **argd)
        if data == None:
    ...
    def showWatchingNow(title, *args, **argd):
        data = traktJsonRequest('GET', '/show/watchingnow.json/%%API_KEY%%/'+str(title), *args, **argd)
        if data == None:
    ...
    def showWatchlist(shows, *args, **argd):
        data = traktJsonRequest('POST', '/show/watchlist/%%API_KEY%%', {'shows': shows}, *args, **argd)
        if data == None:
    ...
    def showsTrending(*args, **argd):
        data = traktJsonRequest('GET', '/shows/trending/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userCalendarShows(username, date=None, days=None, *args, **argd):
        ...
        if date is not None:
            ...
            if days is not None:
                ext += "/"+str(days)
        data = traktJsonRequest('GET', '/user/calendar/shows.json/%%API_KEY%%/'+str(username)+ext, *args, **argd)
        if data == None:
    ...
    def userFriends(username, extended=None, *args, **argd):
        ext = ""
        if extended is not None: etx = "/"+str(extended)
        data = traktJsonRequest('GET', '/user/friends.json/%%API_KEY%%/'+str(title)+ext, *args, **argd)
        if data == None:
    ...
    def userLibraryMoviesAll(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/movies/all.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryMoviesCollection(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/movies/collection.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryMoviesHated(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/movies/hated.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryMoviesLoved(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/movies/loved.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryShowsAll(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/shows/all.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryShowsCollection(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/shows/collection.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryShowsHated(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/shows/hated.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryShowsLoved(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/shows/loved.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLibraryShowsWatched(username, extended=None, *args, **argd):
        data = traktJsonRequest('GET', '/user/library/shows/watched.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userList(username, slug, *args, **argd):
        data = traktJsonRequest('GET', '/user/list.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userLists(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/lists.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userProfile(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/profile.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatched(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watched.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatchedEpisodes(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watched/episodes.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatchedMovies(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watched/movies.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatching(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watching.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatchlistEpisodes(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watchlist/episodes.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatchlistMovies(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watchlist/movies.json/%%API_KEY%%', *args, **argd)
        if data == None:
    ...
    def userWatchlistShows(username, *args, **argd):
        data = traktJsonRequest('GET', '/user/watchlist/shows.json/%%API_KEY%%', *args, **argd)
        if data == None:
